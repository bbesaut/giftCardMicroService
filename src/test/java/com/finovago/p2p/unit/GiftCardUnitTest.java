...
class GiftCardUnitTest {
    ...
    void should_not_throw_when_card_is_active_and_not_expired() {
        GiftCard card = new GiftCard(merchant, "GC-1", 100.0, true, LocalDate.now().plusDays(30));
    ...
    void should_throw_when_card_is_inactive() {
        GiftCard card = new GiftCard(merchant, "GC-2", 100.0, false, LocalDate.now().plusDays(30));
    ...
    void should_throw_when_card_is_expired() {
        GiftCard card = new GiftCard(merchant, "GC-3", 100.0, true, LocalDate.now().minusDays(1));
    ...
    void should_check_active_before_expiration() {
        GiftCard card = new GiftCard(merchant, "GC-4", 100.0, false, LocalDate.now().minusDays(1));
